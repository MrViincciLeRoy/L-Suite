...
# ?? Database ???????????????????????????????????????????????????????????????????
# DATABASE_URL must be a full connection string, e.g.:
#   mysql://user:pass@host:3306/dbname
#   postgres://user:pass@host:5432/dbname
#
# In production (DEBUG=False) a missing or malformed DATABASE_URL is a hard
# error ? we never silently fall back to SQLite so you don't lose data on
...
def _is_valid_db_url(url):
    ...
    if not url:
        return False
    # Must contain '://' and have something before it (not just '://')
    if '://' not in url:
        return False
    scheme = url.split('://')[0].lower()
    # Reject bare driver names like 'mysql' or 'postgres' with no host/creds
    if not url.split('://')[1]:
        return False
...
if _is_valid_db_url(_DATABASE_URL):
    ...
    # Avian / PlanetScale MySQL requires SSL
    if _db_config.get('ENGINE', '').endswith('mysql'):
        _db_config.setdefault('OPTIONS', {})['ssl'] = {'ssl_disabled': False}
    DATABASES = {'default': _db_config}
elif not DEBUG:
    # In production, refuse to start with no valid DB ? better a clear crash
    # than silently using SQLite and losing all data on the next redeploy.
    raise RuntimeError(
...
# ?? Sessions ???????????????????????????????????????????????????????????????????
# Store sessions in the database so they survive Render redeploys.
# (This is Django's default but stated explicitly for clarity.)
